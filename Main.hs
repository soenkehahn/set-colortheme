...
list :: IO ()
list = do
  let baseDir = flakeInput </> "schemes" </> "base16"
  themes <-
...
getColortheme theme = do
  let themeFile = flakeInput </> "schemes" </> "base16" </> theme <.> "yaml"
...
quickshell theme = do
  copyFromNixStoreIntoHome
    (flakeInput </> "base16-quickshell" </> "quickshell" </> "base16-" <> theme </> "Colors.qml")
    ".config/quickshell/lib/Colors.qml"

nvim :: String -> IO ()
nvim theme = do
  copyFromNixStoreIntoHome
    (flakeInput </> "base16-vim" </> "colors" </> "base16-" <> theme <.> "vim")
...
helix theme = do
  copyFromNixStoreIntoHome
    (flakeInput </> "base16-helix" </> "themes" </> "base16-" <> theme <.> "toml")
...
sway theme = do
  copyFromNixStoreIntoHome
    (flakeInput </> "base16-i3" </> "colors/base16-" <> theme <.> "config")
    ".config/sway/colors"
  colors <- getHexColors theme
  let commands =
        [i|
...
getHexColors theme = do
  readFile (flakeInput </> "base16-i3" </> "colors/base16-" <> theme <.> "config")
    <&> lines
...
rofi theme = do
  copyFromNixStoreIntoHome
    (flakeInput </> "base16-rofi" </> "colors/base16-" <> theme <.> "rasi")
